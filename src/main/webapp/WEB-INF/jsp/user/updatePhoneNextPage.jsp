...
<html>
<head>
    <title>updatePhoneNextPage</title>
...
<form class="layui-form layui-form-pane" id="updatePhoneForm" style="margin-left: 180px; margin-top:45px">
    <div class="layui-form-item">
        <label class="layui-form-label">新手机</label>
        <div class="layui-input-inline">
            <input type="text" name="cellPhone" id="cellPhone" required lay-verify="required" autocomplete="off"  placeholder="请输入新手机号" class="layui-input">
        </div>
    </div>
...
<script>
    ...
    $('#updatePhoneForm').submit(function(e) {
        ...
        $.ajax({
            url: '<%=request.getContextPath()%>/user/updatePhone',
            type: 'POST',
            data: {cellPhone: cellPhone,userId: ${user.id},authCode: authCode},
            success: function(data) {
                if (data == 'success') {
                    ...
                    layer.msg("绑定成功");
                }else if(data == "authCodeError") {
                    alert("验证码错误！");
                }else if(data == "cellPhoneUsed") {
                    alert("手机号已被占用！");
                } else {
                    alert('绑定失败，请重试');
                }
